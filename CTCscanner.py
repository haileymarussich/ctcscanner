...
# Frame
    ...
    agg_from_concat = agg_from_concat.sort_values(by='Agg_Volume', ascending=True)
...
# Filter
    with st.form("my_form"):
        user_text_input = st.text_input("Substring or regex in Name")
        user_text_input = st.text_input("Substring or regex in ID")
        submit = st.form_submit_button("Submit")
        
    if submit:
        concat_df = concat_df[concat_df["Name_y"].astype(str).str.contains(user_text_input)]
        concat_df = concat_df[concat_df["ID_x"].astype(str).str.contains(user_text_input)]
